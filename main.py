...
st_autorefresh(interval=2000, key="vvg_v12_final_fix")
...
def exibir_barra_forca(lista_sinais, titulo):
    # lista_sinais pode ser um dicionário ou uma lista de listas
...
if df1 is not None:
    ...
    exibir_barra_forca(ind1, "FORÇA INDICADORES")
    ...
    exibir_barra_forca(ma1, "FORÇA MÉDIAS")

    st.markdown("---")

    # BLOCO 3: VEREDITO
    c_total = sum(1 for v in ind1.values() if "COMPRA" in v) + sum(1 for m in ma1 if "COMPRA" in m[1])
    total_itens = len(ind1) + len(ma1)
    forca_final = (c_total / total_itens) * 100

    if forca_final > 70: st.success(f"🚀 **COMPRA FORTE ({forca_final:.0f}%)**")
    elif forca_final < 30: st.error(f"📉 **VENDA FORTE ({100-forca_final:.0f}%)**")
    else: st.warning(f"⚖️ **NEUTRO ({forca_final:.0f}%)**")
